...
import { COLOR, FONT, smallCaps, tabularNums } from "../preview-theme";
...
export default function PreviewPricingClient({
  ...
  return (
    <main style={{ maxWidth: 1180, margin: "0 auto", padding: "56px 40px 96px" }}>
      ...
      >
        The Back Bar · Finances · Wholesale pricing
      </p>
      ...
      {/* Assumptions — flat row of inputs, small-caps labels, no box */}
      <section
        style={{
      ...
      >
        {[
          ...
          >
            ...
            <PercentInput
              value={config[key]}
              onChange={(next) => saveConfig({ ...config, [key]: next })}
            />
          </label>
        ))}
      ...
      {/* Status + actions — inline, text-led */}
      <section
        style={{
          display: "flex",
          alignItems: "center",
      ...
      {/* Table — hairline rules, no outer box */}
            ...
            {displayed.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  {/* RRP — click to edit, dotted underline */}
                  <td
                    style={{
                      padding: "18px 12px",
                      textAlign: "right",
                  ...
                  >
                    {isEditing ? (
                      ...
                      <span
                        style={{
                          borderBottom: `1px dotted ${COLOR.ruleBold}`,
                          paddingBottom: 1,
                        }}
                      >
                        {GBP(p.rrp)}
      ...
      >
        {[
          ...
          { label: "Pass rate", value: allPass ? "100%" : `${products.length - failCount} / ${products.length}` },
          {
          ...
          },
          {
            ...
            value:
              (
                products.reduce((s, p) => s + calcMargin(p, config), 0) / products.length
              ).toFixed(1) + "%",
          },
        ].map(({ label, value }) => (
          <div key={label}>
      ...
      <style>{`
        .preview-row:hover { background: ${COLOR.paperDeep}; }
      `}</style>
    </main>
  );
}
